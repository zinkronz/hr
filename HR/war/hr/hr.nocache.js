...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "hr" because\n';
 ...
 * "/hr.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['hr'].superdevmode = true;

  var expectedSuffix = '/hr.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + hr + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:hr';
  var devModeSessionKey = '__gwtDevModeSession:hr';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/hr/hr.nocache.js';
  ...
  $wnd.__gwt_activeModules['hr'].canRedirect = true;
  $wnd.__gwt_activeModules['hr'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/hr?user.agent=' + ua + '&_callback=' + callback;
